...
const Import = ({ onCancel }: Props) => {
  ...
  let detailsCollection = newData.filter((element: Article, index: number) => {
  ...
  const onSubmit = async () => {
    try {
      ...
      const dataNew: Article[] = detailsCollection.filter((item: Article) => item.ProductID !== 0);
      await dispatch(addItemsCollection(dataNew));
      onCancel();
    } catch (error: any) {
      console.error(error);
    }
