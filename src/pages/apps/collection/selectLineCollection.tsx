...
import { addItemsCollection } from 'store/reducers/collections';
import { IndeterminateCheckbox } from 'components/third-party/ReactTable';

import { getProducts } from 'store/reducers/product';

// ==============================|| SELECT PRODUCT Collection - LIST VIEW ||============================== //

export interface PropsSelect {
  onCancel: () => void;
}

const SelectLineCollection = ({ onCancel }: PropsSelect) => {
  ...
  let detailsCollection: any = itemsNew.filter((item: any) => item.isSelected === true);
  ...
  const Accions = () => (
    <Stack direction="row" justifyContent="end" spacing={2} alignItems="end" sx={{ pr: 1 }}>
      ...
      <Button
        variant="contained"
        disabled={detailsCollection && detailsCollection.length === 0}
        onClick={() => {
          dispatch(
            openSnackbar({
              open: true,
          ...
          dispatch(addItemsCollection(detailsCollection));
...
export default SelectLineCollection;