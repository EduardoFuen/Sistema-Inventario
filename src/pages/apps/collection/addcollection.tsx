...
import Import from './importLineCollection';
import SelectLineCollection from './selectLineCollection';
import SummaryTemplate from 'components/SummaryTemplate';
import DetailsCollection from './detailsProduct';

import { useSelector, useDispatch } from 'store';
import { addCollection, resetItemsCollection } from 'store/reducers/collections';
...
function AddCollection () {
  ...
  useMemo(() => dispatch(resetItemsCollection()), [dispatch]);
  ...
  const data = useMemo(
    () => detailsCollection && detailsCollection.length > 0 && summary(detailsCollection, Number(discount) || 0),
    [detailsCollection, discount]
  );
  ...
  return (
    <>
        ...
        <FormikProvider value={formik}>
          <Form autoComplete="off" noValidate onSubmit={handleSubmit}>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12}>
                {detailsCollection && detailsCollection.length > 0 ? (
                  <DetailsCollection />
                ) : (
                  <MainCard>
                    Detalles Productos
                    {detailsCollection.length === 0 && (
                      <FormHelperText error id="personal-supplier-helper">
                ...
                {detailsCollection && detailsCollection.length > 0 && (
                  <MainCard>
                    <Stack direction="row" spacing={2} justifyContent="center" alignItems="center" sx={{ mt: 6 }}>
                      <Typography variant="subtitle1">Cantidad Total: ({detailsCollection.length})</Typography>
                    </Stack>
                  </MainCard>
                )}
              </Grid>
              <Grid item xs={12}>
                {detailsCollection && detailsCollection.length > 0 && <SummaryTemplate data={data} />}
              </Grid>
              <Grid item xs={12}>
      ...
      {/* add Product Collection dialog */}
      <Dialog maxWidth="lg" fullWidth onClose={handleAdd} open={add} sx={{ '& .MuiDialog-paper': { p: 0 } }}>
        {add && <SelectLineCollection onCancel={handleAdd} />}
      </Dialog>
      {/* import product Collection dialog */}
