...
import { deleteItemsCollection, editItemsCollection } from 'store/reducers/collections';
...
const DetailsCollection = ({ product }: any) => {
  const theme = useTheme();
  const dispatch = useDispatch();
  const [inputList, setInputList] = useState<any>([]);
  const { detailsCollection } = useSelector((state) => state.collection);

  const data = product && product?.length > 0 ? product : detailsCollection;
  ...
  const handleInputChange = (e: any, index: number) => {
    ...
    setInputList(list);
    dispatch(editItemsCollection(list));
  };
                  ...
                  <Tooltip title="Delete">
                    <IconButton
                      ...
                      onClick={(e: any) => {
                        ...
                        dispatch(deleteItemsCollection(x.ID));
                      }}
                    >
