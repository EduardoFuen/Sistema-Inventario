...
export function getTotalPurchaseList(page: number = 1) {
  return async () => {
    try {
      dispatch(slice.actions.loading());
      const response = await axios.get(`${HOST}/purchaseTotals`, HEADER);
      if (response.data instanceof Object) {
        //const { Rows, totalRows, totalPages, page }: any = response.data;
...
export function handleSMSDelivery(data: Purchase, message: any) {
  return async () => {
    try {
      dispatch(slice.actions.loading());
      await dispatch(getPurchaseList());
      ...
      const response = await axios.post(`${HOST}/purchase/smsdelivery`, { ...Newdata }, { ...HEADER });
      dispatch(
        openSnackbar({
          open: true,
          message: 'Notificacion a delivery Reenviada.',
          variant: 'alert',
          alert: {
            color: 'success'
          },
